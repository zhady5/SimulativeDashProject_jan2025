...
def create_fig_posts_inds(posts, selected_channel, date_range, bgcolor='#ffb347', word_color = '#666', contr_color = '#f5dfbf', graph_color='#F5DEB3', color_Nx_size='#8B4513'):
    ...
    for i, period in enumerate([('days', 'days', 1), ('weeks', 'weeks', 1), ('months', 'months', 1)]):
        ...
        fig_posts.add_trace(
            go.Indicator(
                value=current,
                title={"text": f"<span style='font-size:1.65em;color:{word_color}'>Публикаций за {period_names[period[0]]}<br><span style='font-size:1.8em;color:gray'>Пред. знач.: {round(previous)}</span>"},
                mode="number+delta",
                number={'font': {'size': 42}},  # Задаем размер шрифта для текущего значения
