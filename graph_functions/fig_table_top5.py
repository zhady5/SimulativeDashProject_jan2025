...
#    return fig
    ...
    html = """ <style> #myTable { width: 100%; border-collapse: collapse; } #myTable th, #myTable td { padding: 8px 0 !important ; text-align: center;  color: #666; font-size: 12px; } .circle { display: inline-block; border-radius: 50%; text-align: center; } </style> """
    html += f"<table id='myTable'><tr><th>ID поста (1)</th><th>{val1}</th><th>ID поста (2)</th><th>{val2}</th><th>ID поста (3)</th><th>{val3}</th><th>ID поста (4)</th><th>{val4}</th></tr>"
        ...
        for i, col in enumerate(df_final.columns):
            ...
            if "ID поста" in col:
                ...
                if pd.notna(value) and pd.notna(link):  # Check that both values are not NaN
                    html += f"<td style='font-size: {font_size}px;' ><a href='{link}' target='_blank' style='color: #666;' >{value}</a></td>"
